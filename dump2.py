...
def main(args: List[str]) -> None:
    if len(args) < 3:
        print("Usage: python script.py <start_path> <output_file> [exclusion_files...] [file_extensions...]")
        print("Both exclusion_files and file_extensions are optional.")
        print("Example: python script.py /path/to/scan output.txt .gitignore exclude.txt .py .js")
        sys.exit(1)
    ...
    for arg in remaining_args:
        if arg.startswith('.'):
            # We've hit the file extensions
            file_types = remaining_args[remaining_args.index(arg):]
            break
        else:
    ...
    if exclusion_files:
        print(f"Using exclusion patterns from {', '.join(exclusion_files)}: {exclusion_patterns}")
    else:
        print("No exclusion files specified. Scanning all files.")
